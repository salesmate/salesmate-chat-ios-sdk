...
class ChatTopWithLogo: ChatTopView {
    ...
    @IBOutlet private weak var userView: AvailableUsersView!
    @IBOutlet private weak var userNameStack: UIStackView!
    ...
    fileprivate override func display() {
        ...
        userNameStack.arrangedSubviews.forEach { $0.removeFromSuperview() }

        for name in viewModel.availableuserViewModel?.names ?? [] {
            let label = UILabel()

            label.font = UIFont.systemFont(ofSize: 14)
            label.textColor = backgroundColor?.foregroundColor
            label.text = name
            label.textAlignment = .center

            userNameStack.addArrangedSubview(label)
        }
