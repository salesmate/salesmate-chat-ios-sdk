...
@interface Rapidops_OpenUDID (Private)
...
@implementation Rapidops_OpenUDID
...
//
+ (NSString*) value {
    return [Rapidops_OpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [Rapidops_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [Rapidops_OpenUDID _generateFreshOpenUDID];
        } else {
...
//        [slotPB setPersistent:YES];
        ...
        if (openUDID && localDict)
            [Rapidops_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
