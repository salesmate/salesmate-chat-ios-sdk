...
@interface RapidopsAPMNetworkLog ()
...
NSString* const kRapidopsReservedEventAPM = @"[RPD]_apm";

@implementation RapidopsAPMNetworkLog

+ (instancetype)logWithRequest:(NSURLRequest *)request andOriginalDelegate:(id)originalDelegate startNow:(BOOL)startNow
{
    if ([RapidopsAPM.sharedInstance isException:request])
        return nil;

    RapidopsAPMNetworkLog* nl = RapidopsAPMNetworkLog.new;
    nl.request = request;
    ...
    if (startNow)
    {
        nl.connectionType = RapidopsDeviceInfo.connectionType;
...
- (void)start
{
    ...
    self.connectionType = RapidopsDeviceInfo.connectionType;
...
- (void)finish
{
    ...
    [Rapidops.sharedInstance recordReservedEvent:kRapidopsReservedEventAPM segmentation:segmentation count:1 sum:self.sentDataSize + self.receivedDataSize duration:self.endTime - self.startTime timestamp:self.startTime];

    Rapidops_LOG(@"APM log recorded:\n%@", self);
}
